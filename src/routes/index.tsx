import { createFileRoute, Link } from "@tanstack/react-router";import { useEffect, useState, type FormEvent } from "react";import { AnimatedSection } from "@/components/AnimatedSection";import { Beaker, Bot, Film, Calculator, Cuboid, PenTool, Sparkles, ArrowRight, FlaskConical, Cpu, Wrench, Palette, Sigma, BookOpen, Star, ChevronDown, RotateCw, PartyPopper, Clock3, CheckCircle2, Loader2 } from "lucide-react";import { motion } from "framer-motion";import { Input } from "@/components/ui/input";import { Textarea } from "@/components/ui/textarea";import { Button } from "@/components/ui/button";import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog";import { toast } from "sonner";import { BlobShape, Squiggle, Star4, Dot, Swirl } from "@/components/Blobs";import { openRegistration } from "@/components/RegistrationModal";import { useServerFn } from "@tanstack/react-start";import { submitDemoDayRegistration } from "@/lib/demo-day.functions";import { demoWorkshops, type DemoWorkshop } from "@/lib/demo-day-workshops";

export const Route = createFileRoute("/")({head: () => ({meta: [{ title: "UFO STEAM HUB — Освіта на дотик" },{ name: "description", content: "Освітній центр STEAM-освіти у Кременчуці. Робототехніка, анімація, 3D моделювання та математика для дітей 6-14 років." },{ property: "og", content: "UFO STEAM HUB — Освіта на дотик" },{ property: "og", content: "Освітній центр STEAM-освіти у Кременчуці" },],}),component: HomePage,});

import heroPhoto from "@/assets/main.jpg";const HERO_PHOTO = heroPhoto;

const courses = [{title: "STEAM-гурток", age: "7-12 років",desc: "Щомісяця — нова тема: світло, магніти, вода, повітря, ґрунт.",details: "Діти досліджують явища через дослід, гру та власні проєкти. Заняття у міні-групах з акцентом на практику.",plan: ["Вступ до теми місяця", "Експерименти та досліди", "Створення власного проєкту", "Презентація результатів"],features: ["Міні-групи до 6 дітей", "Усі матеріали включено", "Сертифікат після завершення"],icon: Beaker, accent: "border-ufo-blue", iconBg: "bg-ufo-blue/10", iconColor: "text-primary",photo: "https://images.unsplash.com/photo-1567168544813-cc03465b4fa8?auto=format&fit=crop&w=600&q=80",},{title: "Робототехніка", age: "6-12 років",desc: "Конструювання, програмування та створення власних роботів.",details: "Від простих механізмів до програмованих роботів. Вивчаємо основи інженерії та алгоритмічного мислення.",plan: ["Основи конструювання", "Знайомство з датчиками", "Програмування рухів", "Змагання роботів"],features: ["Роботи LEGO та Arduino", "Індивідуальний підхід", "Участь у змаганнях"],icon: Bot, accent: "border-ufo-green", iconBg: "bg-ufo-green/10", iconColor: "text-ufo-green",photo: "https://images.unsplash.com/photo-1581092921461-eab62e97a780?auto=format&fit=crop&w=600&q=80",},{title: "Анімація і мультиплікація", age: "7-12 років",desc: "Оживляємо своїх героїв — створюємо мультики від ідеї до озвучки.",details: "Діти придумують персонажів, малюють, анімують та озвучують — повний цикл створення мультфільму.",plan: ["Створення персонажів", "Розкадровка та сценарій", "Анімація кадрів", "Озвучка та монтаж"],features: ["Власний мультфільм", "Розвиток творчості", "Робота з планшетами"],icon: Film, accent: "border-ufo-pink", iconBg: "bg-ufo-pink/10", iconColor: "text-ufo-pink",photo: "https://images.unsplash.com/photo-1513364776144-60967b0f800f?auto=format&fit=crop&w=600&q=80",},{title: "Math&mind", age: "2-6 клас",desc: "Цікава математика, нестандартні задачі.",details: "Олімпіадна математика, логічні задачі, головоломки — розвиваємо математичне мислення через захоплення.",plan: ["Логічні задачі", "Олімпіадні завдання", "Математичні ігри", "Командні змагання"],features: ["Підготовка до олімпіад", "Розвиток логіки", "Ігровий формат"],icon: Calculator, accent: "border-ufo-yellow", iconBg: "bg-ufo-yellow/30", iconColor: "text-primary",photo: "https://images.unsplash.com/photo-1509228468518-180dd4864904?auto=format&fit=crop&w=600&q=80",},{title: "Математика", age: "1-11 клас",desc: "Шкільна математика зрозуміло та ефективно.",details: "Допомагаємо розібратися зі шкільною програмою, заповнити пробіли та підготуватися до контрольних.",plan: ["Діагностика рівня", "Робота з пробілами", "Практика та закріплення", "Підготовка до контрольних"],features: ["Індивідуальна програма", "Зрозумілі пояснення", "Домашні завдання"],icon: PenTool, accent: "border-ufo-blue", iconBg: "bg-ufo-blue/10", iconColor: "text-primary",photo: "https://images.unsplash.com/photo-1596495577886-d920f1fb7238?auto=format&fit=crop&w=600&q=80",},{title: "3D моделювання", age: "7-14 років",desc: "Основи 3D друку, моделювання і ШІ.",details: "Діти створюють 3D-моделі, вивчають основи дизайну та друкують свої вироби на 3D-принтері.",plan: ["Знайомство з 3D-середовищем", "Створення простих моделей", "Складні об'єкти та текстури", "3D-друк власного виробу"],features: ["Друк на 3D-принтері", "Основи ШІ у дизайні", "Власний виріб додому"],icon: Cuboid, accent: "border-ufo-green", iconBg: "bg-ufo-green/10", iconColor: "text-ufo-green",photo: "https://images.unsplash.com/photo-1631544114551-d4cfd9577c41?auto=format&fit=crop&w=600&q=80",},];

import groups from "@/assets/groups.jpg";import steam from "@/assets/steam.jpg";import future from "@/assets/future.jpg";import space from "@/assets/space.jpg";

const features = [{ title: "Міні-групи", desc: "Для дітей 6–14 років, по 4–6 — увага кожному.", photo: groups },{ title: "Безпечне навчання", desc: "Комфортний простір та турботлива команда.", photo: space },{ title: "STEAM підхід", desc: "Наука, технології, мистецтво й проєкти — з сенсом.", photo: steam },{ title: "Навички майбутнього", desc: "Критичне мислення, креативність та командна робота.", photo: future },];

const steamBlocks = [{ letter: "S", word: "Science", desc: "Дослідження та експерименти", icon: FlaskConical, bg: "bg-ufo-blue", text: "text-white" },{ letter: "T", word: "Technology", desc: "Сучасні технології та цифрові навички", icon: Cpu, bg: "bg-ufo-green", text: "text-white" },{ letter: "E", word: "Engineering", desc: "Конструювання та інженерне мислення", icon: Wrench, bg: "bg-ufo-pink", text: "text-white" },{ letter: "A", word: "Art", desc: "Творчість, дизайн і мистецтво", icon: Palette, bg: "bg-ufo-yellow", text: "text-primary" },{ letter: "M", word: "Mathematics", desc: "Логіка, числа та закономірності", icon: Sigma, bg: "bg-primary", text: "text-white" },];

const blobA = "polygon(35% 5%, 65% 8%, 90% 25%, 95% 55%, 80% 85%, 50% 95%, 20% 88%, 5% 60%, 8% 30%)";



function DemoDayRegistrationDialog({workshop,open,onOpenChange,}: {workshop: DemoWorkshop | null;open: boolean;onOpenChange: (open: boolean) => void;}) {const submitRegistration = useServerFn(submitDemoDayRegistration);const [selectedWorkshopId, setSelectedWorkshopId] = useState(workshop?.id ?? "");const [participantName, setParticipantName] = useState("");const [childAge, setChildAge] = useState("");const [phone, setPhone] = useState("");const [consentData, setConsentData] = useState(false);const [consentPhoto, setConsentPhoto] = useState(false);const [submitting, setSubmitting] = useState(false);const [success, setSuccess] = useState(false);

const selectedWorkshop = demoWorkshops.find((item) => item.id === selectedWorkshopId) ?? null;

useEffect(() => {if (!open) return;setSelectedWorkshopId(workshop?.id ?? demoWorkshops[0].id);setSuccess(false);}, [open, workshop]);

function resetForm() {setParticipantName("");setChildAge("");setPhone("");setConsentData(false);setConsentPhoto(false);setSubmitting(false);setSuccess(false);}
...
return (<Dialogopen={open}onOpenChange={(nextOpen) => {onOpenChange(nextOpen);if (!nextOpen) {window.setTimeout(resetForm, 200);}}}><DialogContent className="max-h-[92dvh] overflow-y-auto rounded-3xl sm:max-w-xl p-5 sm:p-6">{success ? (<div className="py-6 text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-ufo-green/15"><CheckCircle2 className="h-9 w-9 text-ufo-green" /></div><DialogTitle className="text-2xl font-semibold text-primary">Дякуємо!</DialogTitle><p className="mt-3 text-sm text-muted-foreground">Реєстрацію отримано. Ми зв’яжемося з вами для підтвердження участі.</p><Buttontype="button"onClick={() => onOpenChange(false)}className="mt-6 w-full rounded-full bg-ufo-yellow text-primary hover/90">Закрити</Button></div>) : (<><DialogHeader><div className="mb-1 flex items-center gap-2 text-ufo-pink"><PartyPopper className="h-5 w-5" /><span className="text-xs font-bold uppercase tracking-[0.18em]">UFO DEMO DAY</span></div><DialogTitle className="text-2xl font-semibold text-primary">Реєстрація на майстерку</DialogTitle><DialogDescription>Оберіть одну активність і залиште контактний номер для підтвердження участі.</DialogDescription></DialogHeader>
...
function HomePage() {const [flipped, setFlipped] = useState<Record<string, boolean>>({});const [activeDemoDate, setActiveDemoDate] = useState<"8 серпня" | "9 серпня">("8 серпня");const [selectedDemoWorkshop, setSelectedDemoWorkshop] = useState<DemoWorkshop | null>(null);const [demoDialogOpen, setDemoDialogOpen] = useState(false);

const scrollToBirthday = () => {const el = document.getElementById("demo-day");if (el) el.scrollIntoView({ behavior: "smooth", block: "start" });};

const openDemoRegistration = (workshop: DemoWorkshop) => {setSelectedDemoWorkshop(workshop);setDemoDialogOpen(true);};

return (<>{/* Hero */}<section className="relative overflow-hidden bg-ufo-cream pt-6 pb-12 md:py-24"><Star4 className="absolute top-6 left-4 md:top-10 md:left-6 opacity-90" color="#f7df5d" size={40} /><Dot className="absolute top-24 left-1/3 opacity-80 hidden md:block" color="#f04770" size={18} /><BlobShape className="absolute -bottom-10 -left-12 opacity-30" color="#17c590" size={220} /><Squiggle className="absolute top-16 right-6 md:top-20 md:right-16 opacity-80" color="#3056dd" size={90} /><Star4 className="absolute bottom-16 right-1/4 opacity-80 hidden md:block" color="#f04770" size={36} />
