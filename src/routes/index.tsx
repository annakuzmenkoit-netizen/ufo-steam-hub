...
import { Users, ShieldCheck, Lightbulb, Brain, Beaker, Bot, Film, Calculator, Cuboid, PenTool, Sparkles, ArrowRight, FlaskConical, Cpu, Wrench, Palette, Sigma, BookOpen, Star, Camera } from "lucide-react";
...
const features = [
  { title: "Міні-групи", desc: "Для дітей 6–14 років, по 4–6 — увага кожному.", gradient: "from-ufo-yellow/40 to-ufo-pink/30" },
  { title: "Безпечне навчання", desc: "Комфортний простір та турботлива команда.", gradient: "from-ufo-blue/30 to-ufo-green/30" },
  { title: "STEAM підхід", desc: "Наука, технології, мистецтво й проєкти — з сенсом.", gradient: "from-ufo-green/40 to-ufo-yellow/30" },
  { title: "Навички майбутнього", desc: "Критичне мислення, креативність та командна робота.", gradient: "from-ufo-pink/30 to-ufo-blue/30" },
];
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-ufo-cream py-16 md:py-24">
        ...
        <div className="absolute top-10 -left-10 w-40 h-40 rounded-full bg-ufo-yellow/40 blur-2xl" />
        <div className="absolute bottom-10 right-10 w-48 h-48 rounded-full bg-ufo-green/20 blur-3xl" />
        <div className="absolute top-1/2 right-1/4 w-24 h-24 rounded-full bg-ufo-pink/20 blur-xl" />
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            {/* Hero blob with photo placeholder */}
            <motion.div
            ...
            >
              <div
                className="absolute inset-0 bg-gradient-to-br from-ufo-yellow via-ufo-pink to-ufo-blue"
                style={{ clipPath: blobA }}
              />
              <div
                className="absolute inset-3 bg-gradient-to-br from-ufo-blue/30 to-ufo-green/30 flex items-center justify-center"
                style={{ clipPath: blobA }}
              >
                <div className="text-center text-white/90 px-6">
                  <Camera className="h-12 w-12 mx-auto mb-2 opacity-80" />
                  <p className="text-sm font-semibold">Фото щасливих дітей</p>
                  <p className="text-xs opacity-80">на занятті</p>
                </div>
              </div>
              {/* Floating accents */}
              <div className="absolute -top-4 -right-4 w-16 h-16 rounded-full bg-ufo-yellow shadow-xl flex items-center justify-center animate-bounce">
                <Sparkles className="h-7 w-7 text-primary" />
              </div>
              <div className="absolute -bottom-4 -left-4 w-12 h-12 rounded-full bg-ufo-green shadow-xl" />
            </motion.div>
          </div>
        </div>
      </section>
                  ...
                  <div className={`rounded-3xl bg-card border-2 ${course.accent} shadow-md hover:shadow-xl transition-all overflow-hidden`}>
                    <AccordionTrigger className="px-5 py-5 hover:no-underline group">
                      <div className="flex items-center gap-4 w-full text-left pr-2">
                        {/* Rounded "image" frame using icon as placeholder */}
                        <div className={`${course.iconBg} rounded-2xl w-16 h-16 shrink-0 flex items-center justify-center shadow-inner`}>
      ...
      {/* Why Us — with photo blob placeholders */}
      <section className="py-20 bg-ufo-cream relative overflow-hidden">
        <div className="absolute top-20 right-0 w-72 h-72 rounded-full bg-ufo-yellow/20 blur-3xl" />
        <div className="absolute bottom-0 left-0 w-64 h-64 rounded-full bg-ufo-pink/10 blur-3xl" />
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              ...
              <AnimatedSection key={f.title} delay={i * 0.1}>
                <div className="text-center">
                  ...
                  <div className="relative mx-auto w-40 h-40 mb-5">
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${f.gradient} shadow-lg`}
                      style={{ clipPath: blobA }}
                    />
                    <div
                      className="absolute inset-2 bg-white/40 backdrop-blur-sm flex items-center justify-center"
                      style={{ clipPath: blobA }}
                    >
      ...
      {/* What is STEAM — 5 distinct colored blocks */}
      <section className="py-20 bg-background">
            ...
            {steamBlocks.map((block, i) => (
              <AnimatedSection key={block.letter} delay={i * 0.08}>
                ...
                >
                  ...
                  <div className="mt-3 inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-sm">
